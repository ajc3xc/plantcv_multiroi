...
def labeled_image_roi(img: np.ndarray=None, labeled_objects_array: np.ndarray=None) -> list[list[tuple[int, int]], int]:
    """Generates roi points based off of a binary image as type numpy array
    Generates roi points and optimal radius size based off of groups in binary image,
    then uses roi_multi to generate the roi points

    Inputs:
    img                             = An RGB or grayscale image to plot the ROI on in debug mode.
    labeled_objects_array           = binary filtered image of img used as input for detecting plants
   ...
   #calculate the euclidean distance between two points
    ...
    center_points, optimal_radius_size = get_roi_centers_and_individual_images()

    #now that this is calculated, use pcv.roi.multi to automatically generate the rois
    rois = pcv.roi.multi(img=img, coord=center_points, radius=optimal_radius_size)
